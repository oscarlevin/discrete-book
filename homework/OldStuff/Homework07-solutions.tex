...
\firstpageheader{Math 228}{\bf Homework 7 Solutions}{Due: Wed, March 13}
...
\begin{parts}
  ...
  \begin{solution}
    ...
    Start with all the different orders, then subtract those which contain more than 4 of one or more items.  First, select one of the 7 items to get 5 or more of (this can be done in ${7 \choose 1}$ ways) then buy 5 of that item.  You still have \$11, so 11 stars and still 6 bars, so you can spend your money in ${17 \choose 6}$ ways.  But now we have over counted, so we must subtract the ways you could buy too much of 2 items.  Pick 2 of the 7 items, spend \$5 on each, and distribute the remaining \$6 among all 7 items.  Then pick 3 of the 7, spend \$5 on each leaving only \$1 to spend on 7 items.
  \end{solution}
...
\begin{parts}
  ...
  \begin{solution}
    ...
    The $6!$ is the total number of ways to move around the labels.  Then we get rid of all the arrangements which have one or more labels on their original present.  ${6 \choose 1}5!$ counts the number of ways to pick one present to get its own label and then arrange the other 5 labels on the other 5 presents.  Then ${6 \choose 2}4!$ gives the number of ways to pick 2 presents to get their own labels and arrange the other 4 labels on the other 4 presents.  We must subtract these because we counted them multiple times already.  But in fact we subtract too much, so add back in the ways to pick 3 presents to get labeled correctly, and the other 3 labels permuted.  And so on.
  \end{solution}
  ...
  \begin{solution}
    ...
    First pick 2 of the 6 presents to get the correct label.  Then we need to take the remaining 4 presents and mislabel them.  That part of the problem is analogous to the previous part, only with 4, instead of 6, presents.
  \end{solution}
