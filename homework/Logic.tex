...
\begin{parts}
  \part Prove that the following is a valid deduction rule:
  \begin{tabular}{rc}
    & $P \imp Q$ \\
    & $Q \imp R$ \\ \hline
    $\therefore$ & $P \imp R$
  ...
  \begin{solution}
    ...
    \begin{tabular}{c|c|c||c|c|c}
      ...
      F   &  F  &  F  &     T     &      T     &      T \\
    \end{tabular}
    Notice that both $P \imp Q$ and $Q \imp R$ are true in rows 1, 5, 7 and 8.  In each of these rows, $P \imp R$ is also true.  So whenever the premises are true, so in the conclusion.  Thus the deduction rule is valid.
  \end{solution}


\part Prove that the following is a valid deduction rule for any $n \ge 2$: 
