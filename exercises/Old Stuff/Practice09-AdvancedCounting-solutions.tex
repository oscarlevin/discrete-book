...
\firstpageheader{Math 228}{\bf Practice Problems 9: Advanced Counting\\ Solutions}{Spring 2012}
...
\begin{parts}
  ...
  \part $P(6, 4) = 6 \cdot 5 \cdot 4 \cdot 3 = 360$, since outputs cannot be repeated.  %How many functions are one-to-one?
  \part None. %How many functions are onto?
...
\begin{parts}
  ...
  \part $P(17, 10)$  %How many functions $f: A \to B$ are one-to-one?
\end{parts}

\question $5^{10} - \left[{5 \choose 1}4^{10} - {5 \choose 2}3^{10} + {5 \choose 3}2^{10} - {5 \choose 4}1^{10}\right]$ %Consider sets $A$ and $B$ with $|A| = 10$ and $|B| = 5$.  How many functions $f: A \to B$ are onto?

\question $5! - \left[{5 \choose 1}4! - {5 \choose 2}3! + {5 \choose 3}2! - {5 \choose 4}1! + {5 \choose 5}0!\right]$.  This is a sneaky way to as for the number of derangements on 5 elements. %Let $A = \{1,2,3,4,5\}$.  How many one-to-one functions $f:A \to A$ have the property that for each $x \in A$, $f(x) \ne x$?

\question ${10 \choose 6}\left(4! - \left[{4 \choose 1} 3! - {4 \choose 2}2! + {4 \choose 3}1! - {4 \choose 4}0!\right]\right)$.  We choose 6 of the 10 ladies to get their own hat, and the multiply by the number of ways the remaining hats can be deranged.   %Ten ladies of a certain age drop off their red hats at the hat check of a museum.  As they are leaving, the hat check attendant gives the hats back randomly.  What is the probability that exactly six of the ladies receive their own hat (and the other four do not)?
...
\question %How many integer solutions are there to the equation $x + y + z = 8$ for which
\begin{parts}
...
\question ${10 \choose 5}$.  We have 5 stars (the five dice) and 5 bars (the five switches between the number 1-6).   %When playing Yahtzee, you roll five regular 6-sided dice.  How many different outcomes are possible from a single roll?  The order of the dice does not matter.

\question ${18 \choose 3}$.  Distribute 10 units to the variables before finding all solutions to $x_1' + x_2' + x_3' + x_4' = 15$ in non-negative integers. %How many integer solutions to $x_1 + x_2 + x_3 + x_4  = 25$ are there fore which $x_1 \ge 1$, $x_2 \ge 2$, $x_3 \ge 3$  and $x_4 \ge 4$?
...
\begin{parts}
  ...
  \part $4^9 - \left[{4 \choose 1}3^9 - {4\choose 2}2^9 + {4 \choose 3}1^9 \right]$.  Now the function from the set of present to the set of kids must be onto. %the presents are unique and each kid gets at least one present?
\end{parts}
\newpage
...
\begin{parts}
  ...
  \part Neither.  If all the kids were identical, and you wanted no empty teams, it would be ${10 \choose 4}$.  Instead, this will be the same as the number of onto functions from a set of size 11 to a set of size 5. 
  \part ${10 \choose 4}$
  \part ${10 \choose 4}$
  \part Neither.  $4!$.
