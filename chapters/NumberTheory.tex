...
\end{defbox}
\footnotetext{If two numbers have no common factors, they are said to be {\em relatively prime}.}
...
\[20x = 23 + 14k\]
or equivalently $20x - 14k = 23$.  This is a linear Diophantine equation\todo{need to define this}, and one for which we can easily see there will be no solution.  The left hand side will always be even, but the right hand side is odd.  A similar problem would occur if the right hand side was divisible by {\em any} number the left hand side was not.  
...
\todo[inline]{We need to define these somewhere}
