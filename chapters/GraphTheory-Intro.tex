...
The problem above, known as the \emph{Seven Bridges of K\"onigsberg}, is the problem that started all of graph theory.   Now consider a ``different'' problem.  Below is a drawing of four dots connected by some lines.  Is it possible to trace over each line once and only once (without lifting up your pencil)?  You must start and end on one of the dots.
