...
The problem above, known as the \emph{Seven Bridges of K\"onigsberg}, is the problem that started all of graph theory.   Consider a ``different'' problem.  Below is a drawing of four dots connected by some lines.  Is it possible to trace over each line once and only once (without lifting up your pencil)?  You must start and end on one of the dots.
...
\begin{center}
\begin{tikzpicture}
\draw[thick] (90:1) \va{$a$} -- (210:1) \vl{$b$} -- (-30:1) \vr{$c$} -- (90:1);
\end{tikzpicture}
\qquad
\begin{tikzpicture}
\draw[thick] (90:1) \va{$u$} -- (210:1) \vl{$v$} -- (-30:1) \vr{$w$} -- (90:1);
\end{tikzpicture}
\end{center}
...
\vskip 1ex
\noindent\textbf{Complete graph}: A graph with edges connecting every pair of vertices.
\vskip 1ex
...
\vskip 1ex
\noindent \textbf{Degree of a vertex}: The number of edges connected to a vertex is called the {\em degree} of the vertex.
\vskip 1ex
...
\vskip 1ex
\noindent \textbf{Planar}: A graph is planar if it is possible to draw it (in the plane) without any edges crossing.
\vskip 1ex
 \noindent\textbf{Subgraph}: We say that $H$ is a subgraph of $G$ if every vertex and edge of $H$ is also a vertex or edge of $G$.  We say $H$ is an {\em induced} subgraph of $G$ if every vertex of $H$ is a vertex of $G$ and for pair of vertices in $H$ are adjacent in $H$ if and only if they are adjacent in $G$.
\vskip 1ex
