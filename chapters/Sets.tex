...
\item This is the set of all integers\index{integers} (positive or negative whole numbers, written $\Z$).  In other words, $\{\ldots, -2, -1, 0, 1, 2, \ldots\}$.  
...
\noindent  \begin{tabular}{p{.75in} p{1.35in} p{3.15in}}
    ...
    \gls{cap} & intersection\index{intersection} & $A \cap B$ is the \emph{set} of all elements which are elements of both $A$ and $B$.\\[1ex]
    \gls{cup} & union\index{union} & $A \cup B$ is the \emph{set} of all elements which are elements of $A$ or $B$ or both.\\[1ex]
...
\noindent  \begin{tabular}{p{.75in} p{1.35in} p{3.15in}}
    ...
    $\forall$ & for all & $\forall x (x \ge 0)$ claims that for every number is greater than 0.\index{universal quantifier} \\[1ex]
    $\exists$ & there exists\index{existential quantifier} & $\exists x (x < 0)$ claims that there are negative numbers (there exists a number such that it is less than 0). \\[1ex]
  \end{tabular}
...
\begin{tabular}{l p{5in}}
  $\emptyset$ & The \emph{empty set}\index{empty set} is the set which contains no elements.\\[1ex]
  $\U$ & The \emph{universe set} is the set of all elements.\\[1ex]
$\N$ & The set of natural numbers\index{natural numbers}. That is, $\N = \{0, 1, 2, 3\ldots\}$ \\[1ex]
$\Z$ & The set of integers\index{integers}.  $\Z = \{\ldots, -2, -1, 0, 1, 2, 3, \ldots\}$\\[1ex]
$\Q$ & The set of rational numbers\index{rationals}.\\[1ex]
$\R$ & The set of real numbers\index{reals}.\\[1ex]
$\pow(A)$ & The \emph{power set}\index{power set} of any set $A$ is the set of all subsets of $A$.
\end{tabular}
\end{defbox}
...
In the example above, $B$ is a subset of $A$.  You might wonder what other sets are subsets of $A$.  If you collect all these subsets of $A$, they themselves for a set -- a set of sets.  We call the set of all subsets of $A$ the \emph{power set}\index{power set} of $A$, and write it $\pow(A)$.  
