...
You might be familiar with the {\em inverse} of a function from algebra or calculus.  Perhaps you want to say something like the inverse of $f(x) = x^3$ is $f\inv(x) = \sqrt[3]{x}$.  You wouldn't be wrong.  The inverse of $f$ is another function $f\inv$ which takes each output of $f$ as an input, and outputs the corresponding input from $f$.  So if $f(3) = 7$, the $f\inv(7) = 3$.  

Can we always find the inverse of a function?  The answer is no.  What could go wrong?  Consider a function $f:X \to Y$.  We want another function $f\inv:Y \to X$.  For $f\inv$ to be a function, we need every element of $Y$ to get mapped to exactly one element of $X$ (in fact, the element of $X$ which $f$ sends to that element of $Y$).  But what if $f$ is not surjective?  Then there are elements of $Y$ which never get hit, so we have nothing to send them back to.  What if $f$ is not injective?  Then there are elements of $Y$ which have two corresponding inputs from $X$, so we would have to send the element of $Y$ to {\em two} elements of $X$ at once.  

As you can see, unless a function is both surjective and injective (i.e., unless it is bijective), there is no hope of finding an inverse function.  Yet it is still useful to be able to talk about the elements of the domain that get sent to particular elements of the codomain.  This is the same sort of thing that an inverse function does, we just ignore that it might not be a function.

Actually, we define $f\inv(y)$ to be a {\em set}!  It is exactly the set of elements of $X$ for which $f(x) = y$.  In symbols, given $f:X \to Y$, we have $f\inv(y) = \{x \in X \st f(x) = y\}$.  We call $f\inv(y)$ the {\em complete inverse image of $y$ under $f$}, or simply the {\em pre-image of $y$ under $f$}

\begin{example}
Let $f:\{1,2,3\} \to \{1,2,3,4\}$ be given by $f(1) = 4$, $f(2) = 4$ and $f(3) = 2$.  Find $f\inv(y)$ for each $y \in \{1,2,3,4\}$.

\begin{solution}
$f\inv(1) = \emptyset$ and $f\inv(3) = \emptyset$ since there is nothing that gets sent to 1 or 3 under $f$. Next, $f\inv(2) = \{3\}$.  This is not the same as saying $f\inv(2) = 3$ (which is wrong).  Remember, we are looking for sets here. Finally, $f\inv(4) = \{1,2\}$.
\end{solution}
\end{example}
...
\begin{itemize}
  ...
  \item A function is {\em injective} (an {\em injection} or {\em one-to-one}) if every element of the codomain is the output for \textbf{at most} one element from the domain.
  \item A function is {\em surjective} (a {\em surjection} or {\em onto}) if every element of the codomain is the output of \textbf{at least} one element of the domain.
  \item A {\em bijection} is a function which is both an injection and surjection.  In other words, if every element of the codomain is the output of \textbf{exactly one} element of the domain.
