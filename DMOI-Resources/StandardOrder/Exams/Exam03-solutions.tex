...
\firstpageheader{Math 228}{\bf\large Exam 3 \\ Solutions}{November 20, 2017}
...
% Consider the statement: ``If a graph has chromatic number at most 4, then it is planar or has an Euler path.''
\begin{parts}
\part[12] Complete a truth table for the statement: $(E \wedge O) \imp C$

% \renewcommand{\arraystretch}{1.5}
%   \begin{tabular}{c|c|c||c}
%
%   $E$ & $O$ & $C$ & \hspace{5 in} \\ \hline
%   T & T & T & \\
%   T & T & F & \\
%   T & F & T & \\
%   T & F & F & \\
...
\part[8] Your friend believes the statement is false, and gives $K_{3,3}$ as a counterexample.  What row in the truth table does $K_{3,3}$ correspond to?  \underline{\bf Use the truth table} to explain why $K_{3,3}$ is NOT a counterexample to the statement.
\begin{solution}
$K_{3,3}$ has an even cycle, has no odd cycles, and has chromatic number 2 (so not at least 5).  This corresponds to the 4th row of the truth table.  But the statement is true in that case, so $K_{3,3}$ does not prove the statement is false.
\end{solution}

\vfill

\part[6] Prove that the statement is indeed false by giving a correct counterexample.  Explain.
\begin{solution}
	We need a graph that has an even cycle, an odd cycle, but does not have chromatic number at least 5 (row 2 in the truth table).  There are lots of choices here for example, we could use a disjoint 3-cycle and 4-cycle.  Or a ``house'' woudl also work.
\end{solution}
...
\vfill
\part Does the graph contain an Euler path?  Find one or say specifically what goes wrong that prevents one.
\begin{solution}
...
\question Consider (connected) graphs in which every vertex has degree 9.  Let $v$ be the number of vertices in such a graph.
\begin{parts}
  \part[6] Suppose $v = 12$.  How many \emph{edges} does the graph have.  Briefly explain.
  \begin{solution}
   54.  Each of the 12 vertices has degree 7, so the sum of the degrees is $12 \cdot 9 = 108$.  But the sum of the degrees counts each edge twice since each edge is attached to exactly two vertices.  So the number of edges is half the sum of the degrees, so 54.
  \end{solution}

  \vfill
  \part[8] Which of the following numbers of vertices are also possible?  \underline{Circle} all that are possible and \underline{cross out} all that are impossible.  Briefly explain your answers.
  \[v = 8  \hspace{1in} v = 10 \hspace{1in} v = 11 \hspace{1in} v = 103\]
  \begin{solution}
   Since this is a graph and not a multigraph, we must have $V$ at least 10 (this would give $K_{10}$).  Additionally, $v$ cannot be any odd number -- if we had an odd number of vertices, then the sum of the degrees would be odd, which is impossible, since the sum of the degrees is {\em twice} the number of edges.  Thus the only one above that is possible is $v = 10$.
  \end{solution}

  \vfill
  \part[10] Again, let $v = 12$.  Prove that the graph is NOT planar.  Your proof should be written out in words and it should be clear what style of proof you are using.
  \begin{solution}
   We have seen that the graph has 12 vertices and 54 edges.  Now if the graph were planar, it would satisfy Euler's formula, $v - e + f = 2$, which would say that there must be 44 faces.  However, since the graph is not a multigraph, each face would be bounded by at least 3 edges, with each edge part of the boundary for two faces.  So we would have
   \[f \le \frac{2e}{3}\]
   But we have 44 faces and 54 edges, which would say that $44 \le 36$, a contradiction.  (We must have 44 faces and at the same time have no more than 36: impossible.)  Therefore the graph is not planar.
  \end{solution}
