...
\firstpageheader{Math 228}{\textbf{Graph Sameness}}{Wednesday, August 22}
...
%\vskip 1em
\noindent\textbf{Main Question}: What does it mean for two graphs to be the same?
...
\question You might also think of a graph as a set of vertices and a set of edges, where each edge is a set of two vertices. 
\vskip 1ex
...
\begin{solution}
The graphs are not equal (they have different sets for $V$) but they \emph{are} isomorphic.  This is fairly easy to see if you draw both graphs, and that suggests how to create the isomorphism (the relabeling function).
...
\question Two different bipartite graphs with 5 vertices and 4 edges
\begin{solution}
	...
	\begin{tikzpicture}
		...
		\coordinate (r2) at (1, -.5);
		\coordinate (r3) at (1, .5);
		\coordinate (r4) at (1, 1);
		\foreach \x in {1,...,4}{
		\draw (l1) -- (r\x);
		\fill (r\x) \v;
		}
	...
	\hspace{3em}
	\begin{tikzpicture}
		\coordinate (l1) at (0,-.5);
		\coordinate (l2) at (0,.5);
		\coordinate (r1) at (1,-.75);
		\coordinate (r2) at (1, 0);
		\coordinate (r3) at (1, .75);
		\draw (r1) \v -- (l1) -- (r2) -- (l2) -- (r3);
		\fill (r1) \v (r2) \v (r3) \v (l1) \v (l2) \v;
	\end{tikzpicture}
\end{center}
\end{solution}
\vfill
