...
\firstpageheader{Math 228}{\bf Euler Paths}{April 13, 2012}
...
An Euler path is a path which uses every edge exactly once.  An Euler circuit is an Euler path which starts and stops at the same vertex.  Your goal is to find a quick way to check whether a graph has an Euler path and/or circuit.
...
\question For each of the graphs below, add at least 3 edges to get graphs which:
\begin{multicols}{3}
 \begin{parts}
  \part Has an Euler circuit
  
  \begin{tikzpicture}[scale=1.5]
   \draw[thick] (0,0) \v -- (1,1) \v -- (0,2) \v -- (-1,1) \v -- (0,0) -- (0,1) \v -- (0,2);
  \end{tikzpicture}

    \part Has an Euler path, but not a circuit
...
\begin{center}
 \begin{tikzpicture}[yscale=.5, scale=2]
 \draw[thick] (-1,-2) \v to [out=120, in=240] (-1,0) \v to [out=120, in=240] (-1,2) \v to [out=300, in=60] (-1,0) to [out=300, in=60] (-1,-2);
  \draw[thick] (1,0) \v -- (-1,2) (-1,0) -- (1,0) -- (-1,-2);
  \end{tikzpicture}
\end{center}
